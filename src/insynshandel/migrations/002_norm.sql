...
-- DELETEd and rebuilt in full on every `insyn normalize` (§5.0). It holds no
-- state of its own. The derived columns (sign … gross_value_sek) are left NULL
-- here and written by `insyn aggregate` (§5.1).
CREATE TABLE transaction_norm (
  ...
  -- Written by `insyn aggregate`, NOT by `insyn normalize` (§5.1, invariant 6).
